...
Get the last release of a github project.
...
usage_error() {
  cat >&2 <<'eof'
Error. Usage: glr --short | --long | --help
eof
  exit 1
}
...
# line 4: print the first word of each line (i.e.: the release version).
  ver="$(gh release list --exclude-drafts --exclude-pre-releases --limit 1 | \
    tail -n +1 | \
    head -n 1 | \
    awk '{print $1}')"
  if [ "$ver" = "" ]; then
